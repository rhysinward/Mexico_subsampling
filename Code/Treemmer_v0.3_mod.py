	...
	while warning != 0:
		...
		if (arguments.list_meta and dict_meta[leaf_to_prune]):			#check if leaf is protected
			...
			if warning == 1:
				...
				if not (arguments.protect_pair and is_sister):
					# check sister if protect_pair is not specified
					if arguments.verbose > 1:
...
parser.add_argument('-lmc','--list_meta_count', metavar='path/to/file', default="",help='path to file. Format for each line: "tag,number", this option is alternative to -mc and allows to specify the different minimum number of leaves that should be retained for different categories',type =str, nargs='?')
...
parser.add_argument('-pp', '--protect_pair', default=False, action='store_true', help='both leaves in an identified couple are protected as long as one of the leaves is protected (default: FALSE => ')
...
if arguments.verbose > 0:
	...
	if (arguments.protect_pair):
		print("\nBoth leaves in an identified pair are protected as long as one of the leaves is protected")
